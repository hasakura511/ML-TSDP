...
def getChartDB():
    dbPath = './web/tsdp/db_charts.sqlite3'
    readConn = sqlite3.connect(dbPath)
    return readConn
...
for lookback in lookbacks:
    ...
    title='Heatmap Lookback {}: {} to {}'.format(lookback, data.index[-lookback-1].strftime('%Y-%m-%d'), data.index[-1].strftime('%Y-%m-%d'))
    ax.set_title(title)
    ...
    futuresMatrix.index.name= title
    tablename='heatmap_'+str(lookback)
    if savePath != None:
        filename =savePath+tablename+'.png'
        fig.savefig(filename, bbox_inches='tight')
        print 'Saved', filename
        
    futuresMatrix.to_sql(name=tablename,con=readPriceConn, index=True,
                if_exists='replace')
