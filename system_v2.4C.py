...
for ticker in livePairs:
    ...
    #Model Parameters
    #validationPeriods = [25,50,100,200,600,900]
    validationPeriods = [500]
    ...
    for validationPeriod in validationPeriods:
        ...
        #check if validation period is greater than dataset
        if t_start_loc+validationPeriod >= dataSet.index.shape[0]:
             validationFirstYear =dataSet.index[-1]
        else:
        ...
        while endOfData<2:
            ...
            if endOfData<2:
                print 'Best Params Found... Simulating Next Period..'
                ...
                runName = 'Next'+str(validationPeriod)+'_'+DF1_BMrunName
                model_metrics, sstDictDF1_[runName] = wf_classify_validate(dataSet, dataSet, [m], model_metrics,\
                                                    wf_is_period, metaData, PRT, showPDFCDF=showPDFCDF)
                compareEquity(sstDictDF1_[runName],runName)
                ...
                if runDPS:
                    if dpsRunName.find('wl') != -1:
                        print 'Elapsed time: ', round(((time.time() - start_time)/60),2), ' minutes'
                        print 'Finished Model Training... Beginning Dynamic Position Sizing..'
                        sst_bestModel = pd.concat([sstDictDF1_[DF1_BMrunName],sstDictDF1_[runName]], axis=0)

                        #calc DPS
                        DPS = {}
                        DPS_both = {}

                        startDate = sstDictDF1_[runName].index[0]
                        endDate = sst_bestModel.index[-1]
                        #for ml in maxLeverage:
                        PRT['maxLeverage'] = float(dpsRunName[dpsRunName.find('wl'):].split()[1][4:])
                        #for wl in windowLengths:
                        wl=float(dpsRunName[dpsRunName.find('wl'):].split()[0][2:])
                        dpsRun, sst_save = calcDPS2(runName, sst_bestModel, PRT, startDate,\
                                                                    endDate, wl, 'both', threshold=CAR25_threshold)
                        DPS_both[dpsRun] = sst_save
                        ...
                        dpsRunName, bestBothDPS = findBestDPS(DPS_both, PRT, sst_bestModel, startDate,\
                                                                    endDate,'both', runName, yscale='linear',\
                                                                    ticker=ticker,displayCharts=showAllCharts,\
                                                                    equityStatsSavePath=equityStatsSavePath)
                        bestBothDPS.index.name = 'dates'
                        bestBothDPS = pd.concat([bestBothDPS, pd.Series(data = dpsRunName,index=bestBothDPS.index, name='system')\
                        ...
                        if validationPeriod not in validationDict:
                            validationDict[validationPeriod] =    pd.concat([sstDictDF1_[runName],\
                                                                            pd.Series(data=1.0, name = 'safef', index = sstDictDF1_[runName].index),
                                                                            pd.Series(data=np.nan, name = 'CAR25', index = sstDictDF1_[runName].index),
                                                                            pd.Series(data=np.nan, name = 'dd95', index = sstDictDF1_[runName].index),
                                                                            pd.Series(data=np.nan, name = 'ddTol', index = sstDictDF1_[runName].index),
                                                                            pd.Series(data=dpsRunName, name = 'system', index = sstDictDF1_[runName].index)
                                                                            ],axis=1)
                        else:              
                            validationDict[validationPeriod] =validationDict[validationPeriod].append(                                                                         
                                                                              pd.concat([sstDictDF1_[runName],\
                                                                            pd.Series(data=1.0, name = 'safef', index = sstDictDF1_[runName].index),
                                                                            pd.Series(data=np.nan, name = 'CAR25', index = sstDictDF1_[runName].index),
                                                                            pd.Series(data=np.nan, name = 'dd95', index = sstDictDF1_[runName].index),
                                                                            pd.Series(data=np.nan, name = 'ddTol', index = sstDictDF1_[runName].index),
                    ...
                    if validationPeriod not in validationDict:
                        validationDict[validationPeriod] =sstDictDF1_[runName]
                    else:              
                        validationDict[validationPeriod] =validationDict[validationPeriod].append(sstDictDF1_[runName])
                


    '''            
    print 'Scoring Validation Curves...'               
    if runDPS:
    ...
    print 'Best Validation Curve Found...'    
    BestEquity = calcEquity_df(validationDict[bestModel.params][['signals','gainAhead']],\
                                            bestModel.C25sig, leverage = validationDict[bestModel.params].safef.values)
    
    bestModel = bestModel.append(pd.Series(data=datetime.datetime.fromtimestamp(time.time())\
                                    .strftime('%Y-%m-%d %H:%M:%S'), index=['timestamp']))
    bestModel = bestModel.append(pd.Series(data=bestModel.params, index=['validationPeriod']))
    ...
    if ticker + '.csv' not in files:
        ...
        BMdf = pd.read_csv(bestParamsPath+ticker+'.csv').append(bestModel, ignore_index=True)
        #BMdf.index = BMdf.timestamp
        BMdf.to_csv(bestParamsPath+ticker+'.csv', index=False)
